...
def perf_flux(
    ...
    if TP_GROUP.rank() == 0:
        print(f'args: {args.G}, {max_m}, {n_dim}, {topk}')
    TP_GROUP.barrier()
...
def test_torch(
    ...
    out = moe_gather_rs_forward_torch(
        TP_GROUP,
        args.M,
        eid_start,
        ep_rank_m_start,
    ...
    E, K, N = weights.shape
    M = m_max//topk
    expert_output = torch.zeros(m_max, K, dtype=inputs.dtype, device=inputs.device)
    expert_unpermuted = torch.zeros(M, topk, K, dtype=inputs.dtype, device=inputs.device)
    if rank == 1:
        print('*'*50)
    ...
    for exp_id in range(E):
        exp_w = weights[exp_id]
        Mi = split_cpu[exp_id + eid_start]
        exp_input = inputs[acc : acc + Mi]
        expert_output[acc : acc + Mi] = torch.matmul(exp_input, exp_w.t())
        acc += Mi
    ...
    # 4. all reduce to get full results
    ...
    if rank == 0:
        print('*'*50)
        print(f'expert output shape: {expert_output.shape}, unpermuted: {expert_unpermuted.shape}, topk reduce shape: {topk_reduce.shape}')
    ...
    if rank == 0:
        ...
        print(f"topk reduce: {topk_reduce.size()}")
        print(f"torch out: {out.size()}")

        if flux.testing.bitwise_eq(topk_reduce, out):
            print(f"✅ torch-native and torch bitwise match")
        else:
            print(f"❌ torch-native and torch not bitwise match")

        try:
            flux.torch_allclose(topk_reduce, out, atol=atol, rtol=rtol)
        except Exception as e:
            print("❌ torch native and torch not matches")
            raise e
        else:
            print("✅ torch native and torch matches")
    TP_GROUP.barrier()
...
if __name__ == "__main__":
    ...
    def check_result():
        ...
        if flux.testing.bitwise_eq(flux_output, torch_output):
            print(f"✅ flux and torch bitwise match")
        else:
            print(f"❌ flux and torch not bitwise match")

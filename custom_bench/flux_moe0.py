...
def init_ep_group(ep_size: int):
    ...
    if TP_GROUP.rank() == 0:
        print(f"rank={TP_GROUP.rank()}, EP_GROUP: {ep_groups}")
...
@torch.no_grad()
def test_torch(ctx: MoeMlp1Ctx, warmup_iters: int, iters: int, gather_input: bool = True):
    # print
    moe_input = ctx.inputs  # all-gathered
    w1 = ctx.weights[0]
    M, hidden_dim = moe_input.shape
    token_output = torch.zeros(M, args.topk, w1.shape[1], dtype=moe_input.dtype, device=moe_input.device)
    if TP_GROUP.rank() == 0:
        print('*'*20)
        print(ctx.naive_impl)
        print(moe_input.shape, w1.shape, ctx.choosed_experts.shape, token_output.shape)
        print(moe_input.dtype, w1.dtype, ctx.choosed_experts.dtype)
        print(f'topk={ctx.topk}, ntokens={ctx.ntokens}, gather_index={ctx.gather_index.shape}, scatter_index={moe_ctx.scatter_index.shape}')
        # print(ctx.gather_index)
        # print(ctx.scatter_index)
        print(f'scatter-input={ctx.scatter_inputs.shape}')
        print(f'nexpert: {ctx.nexperts_ep}')
        print(f'splits: {ctx.splits_cpu} - {sum(ctx.splits_cpu)}')
    TP_GROUP.barrier()

    # NOTE 1. token centric: for each token -> for each expert -> gemm
    for i in range(M):
        token_input = moe_input[i]  # [hidden_dim]
        expert_ids = ctx.choosed_experts[i]  # [top_k]
        
        for j in range(args.topk):
            expert_id = expert_ids[j]

            # Apply expert transformation: w1[expert_id] @ token_input
            expert_output = torch.matmul(w1[expert_id], token_input)  # [intermediate_dim]
            token_output[i][j] = expert_output
    
    # NOTE 2. expert centric: for each expert -> all tokens -> gemm
    expert_output = torch.zeros(M * args.topk, w1.shape[1], dtype=moe_input.dtype, device=moe_input.device)
    offset = 0
    for exp_id in range(ctx.nexperts_ep):
        nxt_offset = offset + ctx.splits_cpu[exp_id]
        if nxt_offset - offset > 0:
            exp_input = ctx.scatter_inputs[offset:nxt_offset]
            # if TP_GROUP.rank() == 0:
            #     print(f'exp_id={exp_id}, offset={offset}, nxt_offset={nxt_offset}, exp_input.shape={exp_input.shape}')
            # TP_GROUP.barrier()
            out = expert_output[offset:nxt_offset]
            torch.matmul(exp_input, w1[exp_id].t(), out=out)
            offset = nxt_offset

    # unpermute back to original order
    expert_unpermuted = torch.zeros(M, args.topk, w1.shape[1], dtype=moe_input.dtype, device=moe_input.device)
    for i in range(M):
        indices = ctx.scatter_index[i]
        # print(expert_output[indices].shape, expert_unpermuted[i].shape)
        expert_unpermuted[i] = expert_output[indices]


    TP_GROUP.barrier()
    if flux.testing.bitwise_eq(expert_unpermuted, token_output):
        print(f"✅ token-out vs expert-out bitwise match")
    else:
        print(f"❌ token-out vs expert-out not bitwise match")
    try:
        flux.torch_allclose(expert_unpermuted, token_output, atol=atol, rtol=rtol)
...
if __name__ == "__main__":
    ...
    if TP_GROUP.rank() == 0:
        print('*'*50)
        print(
            f"is_fp8: {is_fp8}, is_s8: {is_s8}, output_dtype: {output_dtype}\n"
            f'inputs={moe_ctx.inputs.shape}, input_shards={moe_ctx.inputs_shard.shape}, scatter_inputs={moe_ctx.scatter_inputs.shape}, {moe_ctx.input_scale}\n'
            f'{len(moe_ctx.weights)}, {len(moe_ctx.outputs)}, {len(moe_ctx.output_scale)}\n'
            f'topk={moe_ctx.topk}, ntokens={moe_ctx.ntokens}, gather_index={moe_ctx.gather_index.shape}, scatter_index={moe_ctx.scatter_index.shape}\n',
            f"Splits:{moe_ctx.splits_cpu.tolist()}, Shape:{moe_ctx.splits_cpu.shape}, Sum:{sum(moe_ctx.splits_cpu.tolist())}\n"
        )
        print('weights: ')
        for w in moe_ctx.weights:
            print(w.shape)
        print('outputs: ')
        for o, oscale in zip(moe_ctx.outputs, moe_ctx.output_scale):
            print(o.shape)
            print(oscale.shape)
        print('*'*50)
        # sorted_gather_index,_  = torch.sort(moe_ctx.gather_index)
        # print(sorted_gather_index)
        # print(moe_ctx.gather_index)

        # print(moe_ctx.scatter_index[:5])
        # print('*'*50)
    ...
    test_torch(moe_ctx, args.warmup_iters, args.iters, args.gather_input)
    # flux.exec_in_rank_order(
    #     TP_GROUP, lambda: check_result(test_outs, perf_result_torch, "torch_test", "torch")
    # )
